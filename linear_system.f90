module linear_system
	...
	subroutine Lin_sys_solver(a,b,x,s_size,method_type)
		...
		integer(4) :: s_size, i, k, num_bufer,j
		integer(4), dimension(s_size) ::  order							! order - vector of actual x indexses, is used for column permutations
		integer(4), dimension(2) :: max_coords								! coords of maxloc in current matrix
		real(16), dimension(s_size+1) :: row_bufer					
		real(16), dimension(s_size, s_size) :: a
		real(16), dimension(s_size, s_size+1) :: new
		real(16), dimension(s_size) :: column_bufer, b, x
		character*8 method_type
